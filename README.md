...
- Actor colour coding - a distinct, colourblind-aware colour per character (off by default)
...
- **No repeats** - every character in a script gets a different colour
- **Derived from the name** - nothing is stored in the script data, and
  adding a role keeps ~99% of existing colours
- **Readable in both themes** - every colour clears WCAG AA on every
  background the app uses, light and dark
- **Colourblind aware** - the palette is solved for worst-case separation
  under protanopia and deuteranopia, not just normal vision
